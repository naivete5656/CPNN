    ...
    def __init__(
        ...
        for _ in range(nheads):
            self.mixers.append(
                SummaryMixing(
                    input_dim=input_dim,
                    dimensions_f=dimensions_f,
                    dimensions_s=dimensions_s,
...
class SummaryTransformer(nn.Module):
    def __init__(
        self, input_dim, depth, nheads, dimensions_f, dimensions_s, dimensions_c
    ):
        super().__init__()
                ...
                nn.ModuleList(
                    [
                        MultiHeadSummary(
                            ...
                            dimensions_s,
                            dimensions_c,
                            dimensions_projection=input_dim,
                        ),
...
class SEQUOIA_VIS(nn.Module, PyTorchModelHubMixin):
    def __init__(self, args, cfg):
        super().__init__()
        num_outputs = cfg.MODEL.output_dim
        ...
        self.transformer = SummaryTransformer(
            input_dim, depth, nheads, dimensions_f, dimensions_s, dimensions_c
        )

        self.to_latent = nn.Identity()
        self.linear_head = nn.Sequential(
            nn.LayerNorm(input_dim), nn.Linear(input_dim, num_outputs)
        )

    def forward(self, x):

        # pe = posemb_sincos_2d(x)
        if self.pos_emb1D.shape[0] != x.shape[1]:
            x = rearrange(x, "b ... d -> b (...) d") + self.pos_emb1D[: x.shape[1]]
        else:
