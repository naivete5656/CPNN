...
from model.comparisons.model_utils import initialize_weights
...
class ILRA(nn.Module):
    def __init__(self, args, cfg):
        # def __init__(
        #     self, feat_dim=1024, n_classes=2, hidden_feat=256, num_heads=8, topk=1
        # ):
        super().__init__()
        num_layers = 2
        ...
        self.pooling = NLP(dim=hidden_feat, num_heads=num_heads, num_seeds=1, ln=ln)
        ...
        if self.prop:
            self.cls_head = nn.Linear(hidden_feat, cfg.MODEL.prop_cls)

    def forward(self, x, wo_log=False):
        x = x.unsqueeze(0)
