...
class MambaConfig:
    ...
    expand_factor: int = (
        2  # E in paper/comments, expand factor to extend the dim of input token
    )
    ...
    def __init__(self, config: MambaConfig):
        ...
        if config.mamba_2d_pad_token == "zero":
            self.pad_token = nn.Parameter(
                torch.zeros(config.d_model), requires_grad=False
            )
        elif config.mamba_2d_pad_token == "trainable":
            self.pad_token = nn.Parameter(
                torch.rand(config.d_model), requires_grad=True
            )
    ...
    def forward(self, x, coords=None, pad_token=None, pos_embs=None):
        if len(x.shape) == 3 and self.config.mamba_2d:
            ...
            if pos_embs is not None:
                pos_embs = F.interpolate(
                    pos_embs.unsqueeze(0).permute(0, 3, 1, 2),
...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        )  # do not change the dim of expanded x
    ...
    def selective_scan(self, x, delta, A, B, C, D):
        ...
        deltaA = torch.exp(
            delta.unsqueeze(-1) * A
        )  # (B, L, ED, N)   # A_bar with ZOH discretization
        deltaB = delta.unsqueeze(-1) * B.unsqueeze(
            2
        )  # (B, L, ED, N) # B_bar with Euler discretization
    ...
    def selective_scan_2D(self, x, delta, A, B, C, D):
        ...
        deltaA = torch.exp(
            delta.unsqueeze(-1) * A
        )  # (BS, H, W, ED, N)   # A_bar with ZOH discretization
        deltaB = delta.unsqueeze(-1) * B.unsqueeze(
            3
        )  # (BS, H, W, ED, N) # B_bar with Euler discretization
    ...
    def selective_scan_seq_2d(self, x, delta, A, B, C, D):
        ...
        for t in range(0, L):
            ...
            if col_id == 0:
                h = (
                    deltaA[:, t] * h + BX[:, t]
                )  # h = A*(state of upper patch) + B*(input of current patch)
            elif row_id == 0:
                h = (
                    deltaA[:, t] * hs[t - image_shape] + BX[:, t]
                )  # h = A*(state of left patch)  + B*(input of current patch)
            else:
                h = (
                    deltaA[:, t] * h + deltaA[:, t] * hs[t - image_shape] + BX[:, t]
                )  # h = A*(state of upper patch)
...
def reconstruct_2d_wsi(feats, coords, max_w, max_h, patch_size, pad_token):
    ...
    w_indexes = w_indexes - torch.min(w_indexes)
    h_indexes = h_indexes - torch.min(h_indexes)
    
    # Filter: keep only h_indexes < max_h
    valid_mask = (h_indexes < max_h - 1) & (w_indexes < max_w - 1)
    h_indexes = h_indexes[valid_mask]
    w_indexes = w_indexes[valid_mask]
