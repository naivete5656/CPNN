...
class OurTrainer(LightningModel):
    ...
    def forward(self, patch, coords, exp, prop=None, slide_name=None, train=True):
        if "proto_reg" in self.version:
            px_scale, prop_pred, exp_template = self.model(patch, coords)
        else:
        ...
        if train and ("scratch" not in self.version):
            ...
            if "strong_prop" in self.version:
                ...
                loss = loss + self.calculate_pcc_loss(exp_template[0], exp_template[1])
            else:
...
class DeconvExp(OurTrainer):
    def __init__(self, model, args, cfg):
        ...
        if "step2" in self.version:
            if "mask" in self.version:
                ckpts = torch.load(
                    f"ckpts/exps/{args.fold}/{args.dataset}-{args.method}_static_mask{args.trainer}{args.data_type}{args.resolution}{args.fold}/best_model.ckpt"
                )["state_dict"]
            else:
                ckpts = torch.load(
                    f"ckpts/exps/{args.fold}/{args.dataset}-{args.method}_mask{args.trainer}{args.data_type}{args.resolution}{args.fold}/best_model.ckpt"
                )["state_dict"]

            # ckpts = torch.load(
            #     f"ckpts/exps/{args.fold}/{args.dataset}-{args.method}_static_mask{args.trainer}{args.data_type}{args.resolution}{args.fold}/best_model.ckpt"
            # )["state_dict"]
            prj_state_dict = {k.replace("model.", ""): v for k, v in ckpts.items() if k.startswith("model")}
            self.model.load_state_dict(prj_state_dict, strict=False)

        # if args.resume is not None:
        #     ckpt = torch.load(
        #         f"ckpts/exps/{args.fold}/{args.resume}{args.fold}/best_model.ckpt",
        #         map_location="cpu",
        #     )["state_dict"]
        #     prj_state_dict = {
        #         k.replace("model.", ""): v
        ...
        #     }
        #     model.load_state_dict(prj_state_dict)
    ...
    def forward(self, patch, coords, exp, prop=None, slide_name=None, train=True):
        ...
        if train and ("scratch" not in self.version) and ("naive" not in self.version):
            prop_loss = self.calculate_prop_loss(prop_pred, prop, slide_name)
            reg_loss = self.calculate_reg_loss()
            if "strong_prop" in self.version:
                loss = loss + self.reg_weight * (1e3 * prop_loss + reg_loss)
            elif "proto_reg" in self.version:
                loss = loss + self.calculate_pcc_loss(exp_template[0].to(loss.device), exp_template[1].to(loss.device))
            else:
                loss = loss + self.reg_weight * (prop_loss + reg_loss)

        if "step2" in self.version:
            reg_loss = self.calculate_pcc_loss(
                torch.log(self.model.org_temp.to(self.model.exp_template)),
                self.model.exp_template,
            )
            loss = loss + reg_loss
